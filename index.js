...
app.get('/cart', isAuthenticated, (req, res) => {
	...
	let sql = `SELECT b.ISBN, b.title, b.price, b.cover, ca.quantity, ca.CartType FROM Book b, CartItems ca WHERE (ca.BookId = b.ISBN) and ca.customerId = ${req.session.user.id} and CartType=0` //AND userId = req.session.id;


	connection.query(sql, (err, rows, results) => {

		if (err) {
			console.log("failed to query cart" + " " + err);
			// res.sendStatus(500);
			return
		} else{
		console.log( rows);
...
//0 is cart - main cart 
//1 is cart type saved for later
//move to cart changes from 1 to 0 
//save for later change from 0 to 1. 

app.get('/SaveForLater', isAuthenticated, (req, res) => {

	let sql = `SELECT b.ISBN, b.title, b.price, b.cover, ca.quantity, ca.CartType FROM Book b, CartItems ca WHERE (ca.BookId = b.ISBN) and ca.customerId = ${req.session.user.id} and CartType=1` //AND userId = req.session.id;
...
app.post('/SaveForLater', isAuthenticated, (req, res) => {
	let query = `update CartItems set cartType=1 where BookId =${req.body.isbn}`
	connection.query(query, (err, results) => {
		if (err) res.status(400).send({error: "Error with retrieving addresses"});
		else res.status(200).send(results);
	});
...
//cart items takes a number
//defining the cart 0 
// defining the cart 1
app.post('/moveToCart', isAuthenticated, (req, res) => {
		let sql = `update CartItems set CartType = 0 cart where BookId=${req.body.isbn}`;
		connection.query(query, req.body.isbn + '', (err, results) => {
			console.log(typeof (req.body.isbn));
			if (err) res.status(400).send({
				error: "this did not" + err
			});
			else res.status(200).send({
				success: "Cart Items removed",
			});
		});
});
...
app.post('/cart', isAuthenticated, (req, res) => {
	let query = `INSERT INTO CartItems (customerId, BookId ) VALUES (${req.session.user.id}, ?)`;
	connection.query(query, req.body.isbn +'', (err, results) => {
		console.log(typeof (req.body.isbn ));
		if (err) res.status(400).send({
			error: "Couldn't save to cart" + err
		})
		else res.status(200).send({
...
app.delete('/cart', isAuthenticated, (req, res) => {

	let query = `DELETE FROM CartItems WHERE customerId = ${req.session.user.id} and BookId = ${req.query.id}`;
	console.log("hello");

	connection.query(query, req.body.isbn + '', (err, results) => {
		console.log(typeof (req.body.isbn));
		if (err) res.status(400).send({
			error: "Error removing the Cart Items" + err
		});
		else res.status(200).send({
			success: "Cart Items removed",
		});
...
// 	let query = `DELETE FROM CartItems WHERE BookId = ${req.session.user.id} and ISBN = ?`;
// 	connection.query(query, req.body.isbn, (err, results) => {
// 		if (err) res.status(400).send({
// 			error: "Error removing the CartItems"
// 		});
// 		else res.status(200).send({
// 			success: "CartItem removed"
// 		});
// 	});
// });